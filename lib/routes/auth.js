...
router
  ...
  .delete('/users/:id/roles/:role', ensureAuth(), ensureRole('admin'), ({ user, params }, res, next) => {
    
    if(user.id === params.id) {
      next({
        statusCode: 401,
        error: 'admin may not remove own admin role'
      })
    } else {
      User.removeRole(user.id, params.role)
        .then(roles => res.json(roles))
        .catch(next);
    }
  })

  .get('/users', ensureAuth(), ensureRole('admin'), (rec, res, next) => {
